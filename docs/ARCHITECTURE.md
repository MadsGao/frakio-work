...
`apps/desktop` owns macOS window behavior, API process lifecycle, logs, approved filesystem dialogs, and approved GitHub Release links. The renderer is sandboxed.
...
GitHub Actions validates the source Web UI on macOS, Windows, and Linux. Tagged releases build separate Apple Silicon and Intel runtime/application artifacts on the matching macOS runners.
